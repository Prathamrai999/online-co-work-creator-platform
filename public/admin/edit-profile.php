...
<?php include('inc/header.php');?>
		...
		if(isset($_GET['id'])){
			$id=$_GET['id'];
			$profile_edit = "SELECT * FROM profile where id = '$id' LIMIT 1";
			$profile_run = mysqli_query($con,$profile_edit);
			
			if(mysqli_num_rows($profile_run)>0)
			{
				$row = mysqli_fetch_array($profile_run);
			}
			else{
				?>
...
<div class="card">			   
	   ...
	   <h2 class="mouse">Update Your Profile</h2>
                        <form action="profile-data-submitted.php" method = "POST" >
                                       <div class="row">
			                         <div class="col-md-4 mb-3">
                                <label class = "form-label" id=formlab >Name
                                </label>
                                <input type="text" name = "name" value =" <?= $row['name'] ?>" class="form-control" id="dark"  required>
                            </div>
                            <div class="col-md-4 mb-3">
